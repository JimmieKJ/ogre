...
protected:
	...
	void setupLights()
	{
		...
		// Create a light node
		node = mSceneMgr->getRootSceneNode()->createChildSceneNode(Vector3(-200, 0, 0));

		// Create a 14 second animation with spline interpolation
		anim = mSceneMgr->createAnimation("Path1", 8);
		...
		track->createNodeKeyFrame(2)->setTranslate(Vector3(0, -200, 0));
		track->createNodeKeyFrame(4)->setTranslate(Vector3(200, 0, 0));
		track->createNodeKeyFrame(6)->setTranslate(Vector3(0, 200, 0));
		track->createNodeKeyFrame(8)->setTranslate(Vector3(-200, 0, 0));
		...
		mTrail->setColourChange(0, 1.0, 1.0, 1.0, 1.0);
		mTrail->setInitialWidth(0, 10);
	...
	//	bbs->setRenderQueueGroup(cPriorityLights);
		Entity* SphereEntity =mSceneMgr->createEntity("ss","sphere.mesh");
		SceneNode* nodechild=node->createChildSceneNode();
		nodechild->attachObject(SphereEntity);
		nodechild->setScale(Ogre::Vector3::UNIT_SCALE*(5.0/SphereEntity->getBoundingRadius()));
