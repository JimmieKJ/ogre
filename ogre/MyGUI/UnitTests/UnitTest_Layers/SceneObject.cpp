...
namespace demo
{
	...
	void SceneObject::updateData()
	{
		...
		Ogre::SceneNode* sNode = entity->getParentSceneNode();
		Ogre::Vector3 vPosition = sNode->getPosition();
		Ogre::Quaternion Quat= sNode->getOrientation();
		Ogre::Vector3 vScale= sNode->getScale();

		if (entity != nullptr && !mMaterialName.empty())	
		{
			mVertexCount = 0;
			mIndexCount = 0;
GetMeshInformation(entity->getMesh(), mVertexCount, mVertices, mIndexCount, mIndices, mTextureCoords, vPosition, Quat, vScale, mMaterialName);
//GetMeshInformation(entity->getMesh(), mVertexCount, mVertices, mIndexCount, mIndices, mTextureCoords, Ogre::Vector3::ZERO, Ogre::Quaternion::IDENTITY, Ogre::Vector3::UNIT_SCALE, mMaterialName);
			...
			if (!material.isNull())
			{
				...
				if (mTextureUnit)
				{
					std::string strnameTextu=mTextureUnit->getTextureName();
					mTextureUnit->setTextureName(mTextureName);
				//	mTextureUnit->getTextureCoordSet()
					//	Ogre::TexturePtr text =mTextureUnit->_getTexturePtr(1)->getWidth();///////////////////////////
					
						// mTextureUnit->getTextureTransform();
