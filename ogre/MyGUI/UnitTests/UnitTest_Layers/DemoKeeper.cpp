...
namespace demo
{
	...
	{
#ifdef MYGUI_OGRE_PLATFORM
		Ogre::Entity* entity = getSceneManager()->createEntity("ControlPanel", "ControlPanel.mesh");//ControlPanel
		Ogre::SceneNode* node = getSceneManager()->getRootSceneNode()->createChildSceneNode();
		node->attachObject(entity);

		Ogre::Entity* cube = getSceneManager()->createEntity("3DGUIPlane","dimian.mesh");//3D Edit//dimian.mesh//3DGUIPlane.mesh
		Ogre::SceneNode* cubenode = getSceneManager()->getRootSceneNode()->createChildSceneNode();//Ä¿Ç°dimian.meshµÄ²ÄÖÊÎÄ¼þ»¹Ã»ÓÐ
		cubenode->attachObject(cube);//ËùÒÔÊÇ°×É«µÄ¡£//´í ¡£²ÄÖÊÔÚplane.materialÖÐ¾ÍÓÐ¡£ÖÁ½ñ²»ÖªµÀÎªÉ¶°×É«µÄ¡£Ô´Âë×ÊÔ´¶¼ÕýÈ·°¡
		
		//cubenode->setScale(1,20,20);
		cubenode->roll(Ogre::Radian(1.57));
		cubenode->yaw(Ogre::Radian(-1.57));
		cubenode->setPosition(8,30,10);
		
// 		Ogre::SceneNode* sn= cube->getParentSceneNode();
// 		sn->getPosition();
// 		sn->getOrientation();
// 		sn->getScale();
// 		Ogre::Matrix4 matrix= sn->_getFullTransform();
// 		matrix.makeTransform();

		Ogre::Entity* ThirdText3D=getSceneManager()->createEntity("3DGUIPlane2","3DGUIPlane.mesh");//Bubble
		ThirdText3D->setMaterialName("BubbleLiuMaterial");
		Ogre::SceneNode* cubenode3 = getSceneManager()->getRootSceneNode()->createChildSceneNode();
		cubenode3->attachObject(ThirdText3D);
			
		cubenode3->setScale(1,40,40);
	cubenode3->setPosition(32,20,20);

// 	Ogre::Entity* ThirdText3D2=getSceneManager()->createEntity("3DGUIPlane23","dimian.mesh");//Bubble
// 	//ThirdText3D2->setMaterialName("BubbleLiuMaterial");
// 	Ogre::SceneNode* cubenode4 = getSceneManager()->getRootSceneNode()->createChildSceneNode();
// 	cubenode4->attachObject(ThirdText3D2);
// 	cubenode4->setPosition(-10,20,10);
	...
	void DemoKeeper::injectMouseMove(int _absx, int _absy, int _absz)
	{
		...
		// ïð?çàæàòî?ïðàâîé âðàùàå?ñöåí?
		if (mRightButtonPressed)
		{
			// îòíîñèòåëüíî?ñìåùåíèå
	...
	void DemoKeeper::injectMousePress(int _absx, int _absy, MyGUI::MouseButton _id)
	{
		...
		{
			// âðàùàå?ñöåí?òîëüêî êîãä?íå íà?ãóåì
